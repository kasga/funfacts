...
// BATH
var i=t.food;foodFormat=i.format,foodNum=i.number;var r=$("#hb-food").html(),d=Handlebars.compile(r),s=d(i);$("#de-container-food").html(s);
// LIVING ROOM
...
// STANDBY
function initStandby(){inView.offset(600),inView("#de-container-standby").once("enter",function(){$(".standby-bubble").each(function(t){setTimeout(function(){$(".standby-bubble").eq(t).addClass("show")},1e3*Math.random())}),setTimeout(initStandbyCTA,1e3)})}function initStandbyCTA(){$(".standby-cta").addClass("init-btn")}function initMobile(){isMobile&&(
// Init sections
...
// Removes the video from the mark up
$("#livingroom-video").remove()}var w,h,isMobile=!1,bathRedBox,bathLeftBox,arrowGfx,videoElement,videoX,bathLeftBoxState="in",dataURL="data.json",foodNum,foodFormat;$(document).ready(function(){w=$(window).outerWidth(),h=$(window).outerHeight(),w<768&&(isMobile=!0),loadData()});
// FOOD
var pct={num:0};